...
//code 
    ...
    var firstnameOld=member[0].first_name
    var lastnameold=member[0].last_name
    var oldAge= member[0].age
    var avg=0
    for (let i = 0 ; i< member.length ; i++){
        avg=avg+member[i].age
    }
    console.log(firstnameOld)
    let res={
        total_member:member.length,
        oldest_member:{
            full_name:`${firstnameOld} ${lastnameold} `,
            age:oldAge
        },
        youngest_member:{
            full_name:`${member[member.length-1].first_name} ${member[member.length-1].last_name}`,
            age:`${member[member.length-1].age}`
        },
        average_member_age:Math.round(avg/member.length)
    }
