...
//   {name: "Buzz Lightyear", harga: 2000, stock: 1}]))
 ...
 function exchangeScore(players) { 
  ...
  let listItem= sortByPrice(listItemz)


  let res = []

     for (let i = 0 ; i < players.length; i ++){
       let countPoint = players[i].points
       let obj = {
         nama:players[i].name,
         items:[]
       }
       for (let j = 0 ; j < listItem.length; j++){
         ...
         if (countPoint>=listItem[j].harga && listItem[j].stock>0){
          countPoint=countPoint-listItem[j].harga
          obj.items.push(listItem[j].name)
          listItem[j].stock=listItem[j].stock-1

          //input dowhile here
          
         }
       
        while (countPoint>listItem[j].harga && listItem[j].stock>0){
          countPoint=countPoint-listItem[j].harga
          obj.items.push(listItem[j].name)
          listItem[j].stock=listItem[j].stock-1
        }
