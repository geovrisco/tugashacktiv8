...
function ongkosKirim(pengirim) {
  ...
  for (let i = 0; i < pengirim.length; i++) {
    ...
    if (
      rute[pengirim[i].asal] != undefined ||
      rute[pengirim[i].tujuan] != undefined
    ) {
      obj.durasi = Math.abs(rute[pengirim[i].asal] - rute[pengirim[i].tujuan]);
      obj.tagihan = obj.durasi * 9000;
      if (pengirim[i].ekspedisi == "NJE") {
        obj.tagihan -= (obj.tagihan * 10) / 100;
      }
    }
    result.push(obj);
  }
